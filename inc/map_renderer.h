...
class MapOutputOperator {
  ...
  MapOutputOperator(const dom_ents::OutputSettings& output_settings)
    : output_settings_(output_settings) {
  }

  ext_libs::svg::Document  MakeDocument(
      const std::map<std::string_view,
      const dom_ents::Route*>& routes) const;

  std::vector<ext_libs::svg::Polyline> MakeWholeRoute(
      const std::map<std::string_view,
      const dom_ents::Route*>& routes,
      const SphereProjector& sphere_projector) const;

  std::vector<ext_libs::svg::Text> MakeRouteLabel(
      const std::map<std::string_view,
      const dom_ents::Route*>& routes,
      const SphereProjector& sphere_projector) const;

  std::vector<ext_libs::svg::Circle> MakeStopSigns(
      const std::map<std::string_view,
      const dom_ents::Stop*> & stops,
      const SphereProjector& sphere_projector) const;

  std::vector<ext_libs::svg::Text> MakeStopLabel(
      const std::map<std::string_view,
      const dom_ents::Stop*> & stops,
      const SphereProjector& sphere_projector) const;
