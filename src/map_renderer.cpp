...
ext_libs::svg::Document MapOutputOperator::MakeDocument(
    const std::map<std::string_view, const dom_ents::Route*>& routes) const {
  ...
  for (const auto& line : MakeWholeRoute(routes, sphere_projector)) {
    doc_result.Add(line);
  }
  for (const auto& text : MakeRouteLabel(routes, sphere_projector)) {
    doc_result.Add(text);
  }
  for (const auto& circle : MakeStopSigns(stops, sphere_projector)) {
    doc_result.Add(circle);
  }
  for (const auto& text : MakeStopLabel (stops, sphere_projector)) {
    doc_result.Add(text);
  }
  return doc_result;
}

std::vector<ext_libs::svg::Polyline> MapOutputOperator::MakeWholeRoute(
    const std::map<std::string_view,
    const dom_ents::Route*>& routes,
  ...
  for (const auto& [name, route] : routes) {
    ...
    std::vector<const dom_ents::Stop*> stops {
        route->stops.begin(), route->stops.end()};
    if (!route->circular) {
      stops.insert(stops.end(),
                   std::next(route->stops.rbegin()),
                   route->stops.rend());
    }
    ...
    for (const auto& stop : stops) {
      segment.AddPoint(sphere_projector(stop->coordinates));
    }
    ...
    (color_number < (output_settings_.color_palette.size() - 1))
    ? ++color_number
    : color_number = 0;
...
std::vector<ext_libs::svg::Text> MapOutputOperator::MakeRouteLabel(
    const std::map<std::string_view,
                   const dom_ents::Route*>& routes_map,
                   const SphereProjector& sphere_projector) const {
  ...
  for (const auto& [name, route] : routes_map) {
    ...
    if ((!route->circular)
      ...
      text_repeat.SetPosition(
          sphere_projector(
                route->stops[route->stops.size() - 1]->coordinates));
      underlayer_repeat.SetPosition(
          sphere_projector(
              route->stops[route->stops.size() - 1]->coordinates));
...
std::vector<ext_libs::svg::Circle> MapOutputOperator::MakeStopSigns(
    const std::map<std::string_view,
    const dom_ents::Stop*>& stops_map,
    const SphereProjector& sphere_projector) const {
...
std::vector<ext_libs::svg::Text> MapOutputOperator::MakeStopLabel(
    const std::map<std::string_view,
    const dom_ents::Stop*>& stops_map,
    const SphereProjector& sphere_projector) const {
  std::vector<ext_libs::svg::Text> result;
...
const dom_ents::OutputSettings MapOutputOperator::GetOutputSettings() const {
