...
void Serialize(business_logic::TransportCatalogue& transport_catalogue,
...
void SerializeOutputSettings(
    const MapOutputOperator& map_output_operator,
    transport_catalogue_pb::TransportCatalogue& transport_catalogue_pb) {
  ...
  output_settings_pb.set_bus_label_font_size(
      output_settings.bus_label_font_size);
  *output_settings_pb.mutable_bus_label_offset() =
      SerializePoint(output_settings.bus_label_offset);

  output_settings_pb.set_stop_label_font_size(
        output_settings.stop_label_font_size);

  *output_settings_pb.mutable_stop_label_offset() =
      SerializePoint(output_settings.stop_label_offset);

  *output_settings_pb.mutable_underlayer_color() =
      SerializeColor(output_settings.underlayer_color);

  output_settings_pb.set_underlayer_width(output_settings.underlayer_width);

  for (const auto& color : output_settings.color_palette) {
    *output_settings_pb.add_color_palette() = SerializeColor(color);
  }
  *transport_catalogue_pb.mutable_output_settings() =
      std::move(output_settings_pb);
}
...
map_pb::ColorVar SerializeColor(const ext_libs::svg::Color& color) {
  ...
  switch (color.index()) {
    case 1: {
      color_pb.set_color(std::get<1>(color));
      break;
    }
    case 2: {
      *color_pb.mutable_rgb() = SerializeRGB(std::get<2>(color));
      break;
    }
    case 3: {
      *color_pb.mutable_rgbo() = SerializeRGBO(std::get<3>(color));
      break;
    }
  }
  return color_pb;
...
void SerializeDistances(
    const business_logic::TransportCatalogue& transport_catalogue,
    transport_catalogue_pb::TransportCatalogue& transport_catalogue_pb) {
  for (const auto& [stops_from_to, distance] :
       transport_catalogue.GetStopDistances()) {
...
graph_pb::Graph SerializeGraph(
  ...
  for (int i = 0; i < transport_router.GetGraph().GetVertexCount(); ++i) {
    ...
    for (const auto& edge_id :
         transport_router.GetGraph().GetIncidentEdges(i)) {
...
transport_router_pb::RoutingSettings SerializeRoutingSettings(
    const TransportRouter& transport_router,
    transport_catalogue_pb::TransportCatalogue& transport_catalogue_pb) {
...
void SerializeRouter(
  ...
  *transport_router_pb.mutable_routing_settings() =
      SerializeRoutingSettings(
        transport_router.GetRouterSettings(), transport_catalogue_pb);
  *transport_router_pb.mutable_graph() =
      SerializeGraph(transport_router, transport_catalogue_pb);
  ...
  *transport_catalogue_pb.mutable_transport_router() =
      std::move(transport_router_pb);
}

// ------------ UNPACK BASE ------------

std::tuple<business_logic::TransportCatalogue,
    MapOutputOperator, TransportRouter,
  ...
  MapOutputOperator map_output_operator =
      DeserializeOutputSettings(output_settings, transport_catalogue_pb);
  TransportRouter router{};

  return { std::move(transport_catalogue),
...
MapOutputOperator DeserializeOutputSettings(
    dom_ents::OutputSettings& output_settings,
    const transport_catalogue_pb::TransportCatalogue& transport_catalogue_pb) {

  const map_pb::OutputSettings& output_settings_pb =
      transport_catalogue_pb.output_settings();
  ...
  output_settings.bus_label_offset =
      DeserializePoint(output_settings_pb.bus_label_offset());

  output_settings.stop_label_font_size =
      output_settings_pb.stop_label_font_size();

  output_settings.stop_label_offset =
      DeserializePoint(output_settings_pb.stop_label_offset());

  output_settings.underlayer_color =
      DeserializeColor(output_settings_pb.underlayer_color());
  ...
  for (int i = 0; i < output_settings_pb.color_palette_size(); ++i) {
    output_settings.color_palette.push_back(
        DeserializeColor(output_settings_pb.color_palette(i)));
  }
  return output_settings;
}

ext_libs::svg::Point DeserializePoint(const map_pb::Point& point_pb) {
  return { point_pb.x(), point_pb.y() };
}

ext_libs::svg::Color DeserializeColor(const map_pb::ColorVar& color_pb) {
  if (color_pb.has_rgb()) {
    return ext_libs::svg::RGB { static_cast<int>(color_pb.rgb().red()),
                                static_cast<int>(color_pb.rgb().green()),
                                static_cast<int>(color_pb.rgb().blue()) };
  }
  else if (color_pb.has_rgbo()) {
    return ext_libs::svg::RGBO{ static_cast<uint8_t>(color_pb.rgbo().red()),
...
void DeserializeStops(
  ...
  for (int i = 0; i < transport_catalogue_pb.stops_size(); ++i) {
    ...
    const transport_catalogue_pb::Stop& stop_pb =
        transport_catalogue_pb.stops(i);

    transport_catalogue.AddStop(stop_pb.name(), {
        stop_pb.coordinates().lat(), stop_pb.coordinates().lng() });
  }
}
...
void DeserializeDistances(
  ...
  for (size_t i = 0; i < transport_catalogue_pb.stop_distances_size(); ++i) {
    const transport_catalogue_pb::Distanse& stop_distances_pb =
        transport_catalogue_pb.stop_distances(i);
    const dom_ents::Stop* from =
        transport_catalogue.FindStop(stop_distances_pb.from());
    const dom_ents::Stop* to =
        transport_catalogue.FindStop(stop_distances_pb.to());
    transport_catalogue.AddToStopDistances(
          from, to, stop_distances_pb.distance());
  }
}
...
void DeserializeRoutes(
  ...
  for (size_t i1 = 0; i1 < transport_catalogue_pb.routes_size(); ++i1) {
    const transport_catalogue_pb::Route& route_pb =
        transport_catalogue_pb.routes(i1);
...
ext_libs::graph::DirectedWeightedGraph<double> DeserializeGraph(
    const transport_catalogue_pb::TransportCatalogue& transport_catalogue_pb) {
  const graph_pb::Graph& graph_pb =
      transport_catalogue_pb.transport_router().graph();
  std::vector<ext_libs::graph::Edge<double>> edges(graph_pb.edges_size());
  std::vector<std::vector<ext_libs::graph::EdgeId>> incidence_lists(
        graph_pb.vertexes_size());
  for (int i = 0; i < graph_pb.edges_size(); ++i) {
    const graph_pb::Edge& edge_pb = graph_pb.edges(i);
    edges[i] = { static_cast<size_t>(edge_pb.from()),
  ...
  for (size_t i = 0; i < incidence_lists.size(); ++i) {
    ...
    for (const auto& id : vertex_pb.edge_ids()) {
...
std::map<std::string, ext_libs::graph::VertexId> DeserializeStopNameToIds(
    const transport_catalogue_pb::TransportCatalogue& transport_catalogue_pb) {
  std::map<std::string, ext_libs::graph::VertexId> stop_name_to_ids;
  for (const auto& stop_name_to_id_pb :
       transport_catalogue_pb.transport_router().stop_with_ids()) {
...
TransportRouter DeserializeRoutingSettings(
    const transport_catalogue_pb::TransportCatalogue& transport_catalogue_pb) {
