...
dom_ents::RouteStats RequestHandler::GetRouteStats(
  ...
  for (size_t i = 0; i < route->stops.size() - 1; ++i) {
    ...
    if (route->circular) {
      actual_route_length +=
          transport_catalogue_.GetStopDistance(from, destination);
      geographic_length +=
          dom_ents::geo::ComputeDistance(
              from->coordinates, destination->coordinates);
    }
    else {
      actual_route_length +=
          transport_catalogue_.GetStopDistance(from, destination) +
              transport_catalogue_.GetStopDistance(destination, from);
      geographic_length +=
          dom_ents::geo::ComputeDistance(
              from->coordinates, destination->coordinates) * 2;
    }
  }

  std::unordered_set<std::string_view> unique_stops;
  for (const auto& stop :
      transport_catalogue_.GetRouteNameToRoute().at(route_name)->stops) {
...
const std::set<std::string> RequestHandler::GetRoutesByStop(
...
const std::optional<ext_libs::graph::Router<double>::RouteInfo> RequestHandler
    ::BuildRoute(std::string_view stop_from, std::string_view stop_to) const {
  return transport_router_.BuildRoute(stop_from, stop_to);
}

const ext_libs::graph::DirectedWeightedGraph<double>& RequestHandler
    ::GetTransportRouterGraph() const {
