...
namespace ext_libs {
	...
	namespace json {
		...
		const std::string& Node::AsString () const {if (!IsString ()) throw std::logic_error("Err: Type isnt string"   ); return std::get<std::string>(*this);}
		int                Node::AsInt    () const {if (!IsInt    ()) throw std::logic_error("Err: Type isnt int"      ); return std::get<int        >(*this);}
		bool               Node::AsBool   () const {if (!IsBool   ()) throw std::logic_error("Err: Type isnt bool"     ); return std::get<bool       >(*this);}
		const Array&       Node::AsArray  () const {if (!IsArray  ()) throw std::logic_error("Err: Type isnt array"    ); return std::get<Array      >(*this);}
		const Dict&        Node::AsMap    () const {if (!IsMap    ()) throw std::logic_error("Err: Type isnt map"      ); return std::get<Dict       >(*this);}
		double             Node::AsDouble () const {if (!IsDouble ()) throw std::logic_error("Err: Type isnt dbl / int"); return IsPureDouble() ? std::get<double>(*this) : AsInt();}
		...
		Document Parse (std::istream& input) {return Document{ParseNode(input)};}

		Node ParseNode(std::istream& input) {
			char c;
			input >> c;

			if      (c == 'n')              { input.putback(c); return ParseNull	(input); }
			else if (c == '"')              {                   return ParseString	(input); }
			else if (c == 't' || c == 'f')  { input.putback(c); return ParseBool	(input); }
			else if (c == '[')              {                   return ParseArray	(input); }
			else if (c == '{')              {                   return ParseDict	(input); }
			else                            { input.putback(c); return ParseNumber	(input); }
		}
		...
		Node ParseNumber(std::istream& input) {
			...
			try {
				if (is_int) {
					try {return std::stoi(parsed_num);}
					catch (...) {}
				}
		...
		Node ParseNull(std::istream& input) {
			...
			for (size_t i = 0; i < null_name.size(); i++) {
				if (null_name.at(i) == input.get()) continue;
		...
		Node ParseBool(std::istream& input) {
			...
			for (size_t i = 1; i < bool_name->size(); i++) {
				if (bool_name->at(i) == input.get()) continue;
		...
		void OutputValue(std::nullptr_t, const OutputContext& ctx) { ctx.out << "null"sv; }

		void OutputValue(std::string value, const OutputContext& ctx) {
			ctx.out << "\""sv;
			for (const char& liter : value) {
				if (liter == '\n') { ctx.out << "\\n"sv; continue;}
				if (liter == '\r') { ctx.out << "\\r"sv; continue;}
				if (liter == '\"') { ctx.out << "\\"sv ;          }
				if (liter == '\t') { ctx.out << "\t"sv ; continue;}
				if (liter == '\\') { ctx.out << "\\"sv ;          }
				ctx.out << liter;
			}
			ctx.out << "\""sv;
		...
		void OutputNode(const Node& node, const OutputContext& ctx) {
			std::visit( [&ctx] (const auto& value) { OutputValue(value, ctx); }, node.GetValue() );
		}
